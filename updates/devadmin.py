...
### python requests module using try/except
def admin_get(endpoint, adminauth):
    try:
        r = requests.get(endpoint, headers=adminauth)
    except requests.exceptions.Timeout:
        print('  ****<HTTPS requests.get error> Timeout ({}) '.format(endpoint))
        return -2, ''
    except requests.exceptions.TooManyRedirects:
        print('  ****<HTTPS requests.get error> TooManyRedirects ({}) '.format(endpoint))
        return -3, ''
    except requests.exceptions.RequestException as e:
        print('  ****<HTTPS requests.get error> ReqeustException {} ({}) '.format(e, endpoint))
        return -4, ''
...
def admin_post(endpoint, basicauth, payload):
    try:
        r = requests.post(endpoint, json=payload, auth=basicauth)
    except requests.exceptions.Timeout:
        print('  ****<HTTPS requests.get error> Timeout ({}) '.format(endpoint))
        return -2, ''
    except requests.exceptions.TooManyRedirects:
...
def admin_delete(endpoint, adminauth):
    try:
        r = requests.delete(endpoint, headers=adminauth)
    except requests.exceptions.Timeout:
        print('  ****<HTTPS requests.get error> Timeout ({}) '.format(endpoint))
        return -2, ''
    except requests.exceptions.TooManyRedirects:
...
def getSubscriptionInfoByBabyid(sku, atoken, babyid):
    ## get account information
    base_url = queryAPIsBaseURL(sku)
    auth_header = {'Authorization':'Bearer '+atoken}
    ### get subscription status
